...
**NOTA BENE**: Un layer vettoriale con una sorgente di dati da remoto inizia il caricamento **asincrono** dei dati nel momento in cui viene inserito in mappa. Questo significa che immediatamente dopo l'aggiunta alla mappa la sorgente risulterà ancora vuota, per cui non è possibile eseguire immediatamente funzioni che richiedono i dati. Ad esempio se volessimo adattare la vista della mappa all'estensione dei dati, dovremo farlo solo dopo esserci assicurati che i dati siano effettivamente arrivati. Per farlo possiamo registrare una callback sull'evento `change` dei `ol.source.Vector`, all'interno della quale possiamo verificare che effettivamente la sorgente sia pronta con i dati completamente caricati:
